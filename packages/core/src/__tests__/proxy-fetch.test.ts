...
describe("proxy fetch helpers", () => {
  ...
  it("prefers explicit llm proxyUrl over environment proxy variables", async () => {
    ...
    const env = {
      ZWORLD_LLM_PROXY_URL: "http://zworld-env-proxy:9910",
  ...
  it("uses ZWORLD_LLM_PROXY_URL before standard HTTPS_PROXY/HTTP_PROXY env vars", async () => {
    const { fetchWithProxy, resolveProxyUrl } = await import("../utils/proxy-fetch.js");
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal("fetch", fetchMock);

    const env = {
      ZWORLD_LLM_PROXY_URL: "http://zworld-proxy:9910",
      HTTPS_PROXY: "http://standard-proxy:9910",
      HTTP_PROXY: "http://http-proxy:9910",
    };

    expect(resolveProxyUrl(undefined, env)).toBe("http://zworld-proxy:9910");
    await fetchWithProxy("https://api.example/v1/models", {}, undefined, env);

    expect(proxyAgentMock).toHaveBeenCalledWith("http://zworld-proxy:9910");
    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.example/v1/models",
      expect.objectContaining({
        dispatcher: { kind: "proxy-agent", url: "http://zworld-proxy:9910" },
      }),
    );
  });
