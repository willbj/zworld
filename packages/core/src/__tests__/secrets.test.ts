...
describe("secrets", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "zworld-secrets-"));
  });
  ...
  describe("loadSecrets", () => {
    it("returns empty when .zworld/secrets.json does not exist", async () => {
  ...
  describe("saveSecrets", () => {
    it("creates .zworld dir and writes secrets file", async () => {
      await saveSecrets(root, {
        services: { deepseek: { apiKey: "sk-deep" } },
      });
      const raw = await readFile(join(root, ".zworld", "secrets.json"), "utf-8");
