...
async function readProjectConfig(root: string): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new Error(
      `zworld.json not found in ${root}.\nMake sure you are inside an ZWorld project directory (cd into the project created by 'zworld init').`,
    );
  }
  ...
  try {
    ...
    throw new Error(`zworld.json in ${root} is not valid JSON. Check the file for syntax errors.`);
  }
}
...
async function applyCliProjectConfig(
  ...
  const envBaseUrl = stringValue(env.ZWORLD_LLM_BASE_URL);
  const envService = stringValue(env.ZWORLD_LLM_SERVICE) ?? (envBaseUrl ? guessServiceFromBaseUrl(envBaseUrl) : undefined);
  const envModel = stringValue(env.ZWORLD_LLM_MODEL);
  ...
  await applyProjectServiceConfig(config, llm, services, input.projectRoot, diagnostics, {
    ...
    envApiKey: allowEnvEndpointOverlay ? stringValue(env.ZWORLD_LLM_API_KEY) : undefined,
    envBaseUrl: allowEnvEndpointOverlay ? envBaseUrl : undefined,
    envProvider: allowEnvEndpointOverlay ? stringValue(env.ZWORLD_LLM_PROVIDER) : undefined,
    cli: input.cli,
    env,
  });
}
...
async function applyLegacyEnvConfig(
  ...
  if (env.ZWORLD_LLM_SERVICE) {
    llm.service = env.ZWORLD_LLM_SERVICE;
    diagnostics.serviceSource = "env";
  } else if (typeof llm.service !== "string" || llm.service.length === 0) {
    llm.service = "custom";
  }

  if (env.ZWORLD_LLM_PROVIDER) llm.provider = env.ZWORLD_LLM_PROVIDER;
  else if (typeof llm.provider !== "string" || llm.provider.length === 0) llm.provider = "custom";
  if (env.ZWORLD_LLM_BASE_URL) llm.baseUrl = env.ZWORLD_LLM_BASE_URL;
  if (env.ZWORLD_LLM_MODEL) {
    llm.model = env.ZWORLD_LLM_MODEL;
    diagnostics.modelSource = "env";
  }
  if (env.ZWORLD_LLM_API_KEY) {
    llm.apiKey = env.ZWORLD_LLM_API_KEY;
...
function applyCommonEnv(
  ...
  if (env.ZWORLD_LLM_TEMPERATURE) llm.temperature = Number.parseFloat(env.ZWORLD_LLM_TEMPERATURE);
  if (env.ZWORLD_LLM_THINKING_BUDGET) llm.thinkingBudget = Number.parseInt(env.ZWORLD_LLM_THINKING_BUDGET, 10);
  if (env.ZWORLD_LLM_PROXY_URL) llm.proxyUrl = env.ZWORLD_LLM_PROXY_URL;
  if (env.ZWORLD_LLM_API_FORMAT) llm.apiFormat = env.ZWORLD_LLM_API_FORMAT;
  if (env.ZWORLD_LLM_STREAM) llm.stream = parseBoolean(env.ZWORLD_LLM_STREAM);
  if (env.ZWORLD_DEFAULT_LANGUAGE) config.language = env.ZWORLD_DEFAULT_LANGUAGE;

  const extraFromEnv: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(env)) {
    if (key.startsWith("ZWORLD_LLM_EXTRA_") && value) {
      extraFromEnv[key.slice("ZWORLD_LLM_EXTRA_".length)] = parseEnvValue(value);
    }
  }
...
function warnIfStudioIgnoresEnv(layers: LLMEnvLayers, diagnostics: MutableDiagnostics): void {
  ...
  if (Object.keys(ignored).some((key) => key.startsWith("ZWORLD_LLM_"))) {
    diagnostics.warnings.push("Studio 运行时不会使用 env 中的 ZWORLD_LLM_* 配置；请在服务配置页保存 Studio 配置。");
  }
}
