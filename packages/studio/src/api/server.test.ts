...
describe("createStudioServer daemon lifecycle", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "zworld-studio-server-"));
    ...
    loadProjectConfigMock.mockImplementation(async () => {
      const raw = JSON.parse(await readFile(join(root, "zworld.json"), "utf-8")) as Record<string, unknown>;
      return {
        ...cloneProjectConfig(),
        ...raw,
        llm: {
  ...
  afterEach(async () => {
    ...
    await rm(join(tmpdir(), "zworld-global.env"), { force: true });
  });
  ...
  it("reports config source and detected env overrides for Studio switching", async () => {
    await writeFile(join(root, ".env"), [
      ...
      "ZWORLD_LLM_BASE_URL=https://project.example.com/v1",
      "ZWORLD_LLM_MODEL=gpt-5.4",
      "ZWORLD_LLM_API_KEY=sk-project",
    ].join("\n"), "utf-8");
    await writeFile(join(tmpdir(), "zworld-global.env"), [
      "ZWORLD_LLM_PROVIDER=openai",
      "ZWORLD_LLM_BASE_URL=https://global.example.com/v1",
      "ZWORLD_LLM_MODEL=gpt-4o",
      "ZWORLD_LLM_API_KEY=sk-global",
    ].join("\n"), "utf-8");
    await writeFile(join(root, "zworld.json"), JSON.stringify({
      ...projectConfig,
      llm: {
  ...
  it("imports detected env config into Studio services without exposing the key", async () => {
    await writeFile(join(tmpdir(), "zworld-global.env"), [
      "ZWORLD_LLM_PROVIDER=openai",
      "ZWORLD_LLM_BASE_URL=https://api.kkaiapi.com/v1",
      "ZWORLD_LLM_MODEL=deepseek-v4-flash",
      "ZWORLD_LLM_API_KEY=sk-global",
    ].join("\n"), "utf-8");
    loadSecretsMock.mockResolvedValue({ services: {} });
  ...
  it("does not probe stale global fallback models for custom services when /models is unavailable", async () => {
    ...
    await writeFile(join(root, ".env"), [
      "ZWORLD_LLM_MODEL=MiniMax-M2.7",
      "ZWORLD_LLM_BASE_URL=https://api.minimax.com/v1",
      "ZWORLD_LLM_API_KEY=sk-minimax",
    ].join("\n"), "utf-8");
  ...
  it("serves generated project cover images without exposing arbitrary files", async () => {
    ...
    const traversal = await app.request("http://localhost/api/v1/project/files/../zworld.json");
  ...
  it("reports async create failures through the create-status endpoint", async () => {
    processProjectInteractionRequestMock.mockRejectedValueOnce(new Error("ZWORLD_LLM_API_KEY not set"));
    ...
    await expect(status.json()).resolves.toMatchObject({
      status: "error",
      error: "ZWORLD_LLM_API_KEY not set",
    });
  });
  ...
  it("handles explicit chat chapter edits outside the ZWorld writing agent", async () => {
  ...
  it("classifies ZWorld parser/tool errors as internal instead of blaming the selected provider", async () => {
    ...
    expect(json.error.message).toContain("ZWorld 内部流程错误");
