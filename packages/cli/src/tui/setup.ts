...
export function buildAutoInitMessages(projectName: string, locale: TuiLocale): {
  ...
  if (locale === "en") {
    return {
      ...
      envTemplateHeader: "# LLM Configuration — run zworld tui to configure interactively",
    };
  }
  ...
  return {
    ...
    initialized: "项目已初始化",
    envTemplateHeader: "# LLM 配置 —— 运行 zworld tui 进行交互式配置",
  };
}

export async function ensureProject(cwd: string): Promise<SetupResult> {
  const configPath = join(cwd, "zworld.json");
...
export async function interactiveLlmSetup(
  ...
  try {
    ...
    const envContent = [
      `ZWORLD_LLM_PROVIDER=${finalProvider}`,
      ...(finalService ? [`ZWORLD_LLM_SERVICE=${finalService}`] : []),
      `ZWORLD_LLM_BASE_URL=${effectiveBaseUrl}`,
      `ZWORLD_LLM_API_KEY=${apiKey.trim()}`,
      `ZWORLD_LLM_MODEL=${model.trim()}`,
    ].join("\n");
...
async function autoInit(cwd: string): Promise<void> {
  ...
  const config = {
    ...
    language: "zh",
    llm: {
      provider: process.env.ZWORLD_LLM_PROVIDER ?? "openai",
      baseUrl: process.env.ZWORLD_LLM_BASE_URL ?? "",
      model: process.env.ZWORLD_LLM_MODEL ?? "",
    },
    notify: [],
    daemon: {
      schedule: {
...
async function checkEnvForKey(envPath: string): Promise<boolean> {
  try {
    ...
    const match = content.match(/ZWORLD_LLM_API_KEY=(.+)/);
...
export async function detectProjectLanguage(projectRoot: string): Promise<string | undefined> {
  try {
    const raw = await readFile(join(projectRoot, "zworld.json"), "utf-8");
...
async function parseEnvModel(envPath: string): Promise<ModelInfo | undefined> {
  try {
    ...
    const key = get("ZWORLD_LLM_API_KEY");
    if (!key || key.includes("your-api-key")) return undefined;
    return {
      provider: get("ZWORLD_LLM_PROVIDER") || "openai",
      model: get("ZWORLD_LLM_MODEL") || "unknown",
      baseUrl: get("ZWORLD_LLM_BASE_URL") || "",
    };
  } catch {
    return undefined;
  }
