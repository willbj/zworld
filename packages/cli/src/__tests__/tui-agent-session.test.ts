...
vi.mock("@zworld/core", async () => {
  const actual = await vi.importActual<typeof import("@zworld/core")>("@zworld/core");
...
describe("tui agent session bridge", () => {
  ...
  beforeEach(async () => {
    projectRoot = await mkdtemp(join(tmpdir(), "zworld-tui-agent-"));
    vi.clearAllMocks();
