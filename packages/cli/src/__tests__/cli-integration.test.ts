...
import { StateManager } from "@zworld/core";
...
const failingLlmEnv = {
  ...
  ZWORLD_LLM_BASE_URL: "http://127.0.0.1:9/v1",
  ZWORLD_LLM_MODEL: "test-model",
  ZWORLD_LLM_API_KEY: "test-key",
};

describe("CLI integration", () => {
  beforeAll(async () => {
    projectDir = await mkdtemp(join(tmpdir(), "zworld-cli-test-"));
  });
  ...
  describe("zworld --help", () => {
    it("prints help with command list", () => {
      const output = run(["--help"]);
      expect(output).toContain("zworld");
  ...
  describe("zworld init", () => {
    it("initializes project in current directory", () => {
      const output = run(["init"]);
      expect(output).toContain("Project initialized");
    });

    it("creates zworld.json with correct structure", async () => {
      const raw = await readFile(join(projectDir, "zworld.json"), "utf-8");
    ...
    it("creates .env file", async () => {
      ...
      expect(envContent).toContain("ZWORLD_LLM_API_KEY");
    });
  ...
  describe("zworld init <name>", () => {
    ...
    it("prints English next steps when initialized with --lang en", async () => {
      const englishDir = await mkdtemp(join(tmpdir(), "zworld-cli-en-init-"));

      try {
        const output = run(["init", englishDir, "--lang", "en"]);
        expect(output).toContain("Project initialized");
        expect(output).toContain("zworld book create --title 'My Novel'");
  ...
  describe("zworld interact", () => {
    it("returns the agent-session JSON contract for natural-language interactions", async () => {
      const initialized = await stat(join(projectDir, "zworld.json")).then(() => true).catch(() => false);
    ...
    it("binds the requested book when interact is called with --book", async () => {
      ...
      try {
        ...
        await rm(join(projectDir, ".zworld-session.json"), { force: true }).catch(() => {});
      }
    }, CLI_PROCESS_TIMEOUT_MS);
  });

  describe("zworld config set-model", () => {
    ...
    it("treats localhost OpenAI-compatible endpoints as API-key optional", async () => {
      ...
      try {
        ...
        await writeFile(envPath, [
          "ZWORLD_LLM_PROVIDER=openai",
          "ZWORLD_LLM_BASE_URL=http://127.0.0.1:11434/v1",
          "ZWORLD_LLM_MODEL=gpt-oss:20b",
          "",
        ].join("\n"), "utf-8");

        const { stdout } = runStderr(["doctor"], {
          env: { ZWORLD_LLM_API_KEY: "" },
        });
