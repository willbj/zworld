...
import { deriveBookIdFromTitle, normalizePlatformOrOther, PipelineRunner, StateManager, type BookConfig } from "@zworld/core";
import {
    ...
    try {
      ...
      if (bookIds.length === 0) {
        if (opts.json) {
          ...
          log("No books found. Create one with: zworld book create --title '...'");
        }
        return;
      }
