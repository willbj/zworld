...
import { deriveBookIdFromTitle, normalizePlatformOrOther, PipelineRunner, type BookConfig, type FanficMode } from "@zworld/core";
    ...
    try {
      ...
      if (opts.json) {
        log(JSON.stringify({
          ...
          nextStep: `zworld write next ${bookId}`,
        }, null, 2));
      } else {
        ...
        log("");
        log(`Next: zworld write next ${bookId}`);
      }
    } catch (e) {
      if (opts.json) {
...
fanficCommand
  ...
  .action(async (bookIdArg: string | undefined, opts) => {
    try {
      ...
      const { StateManager } = await import("@zworld/core");
      ...
      try {
        ...
        throw new Error(`该书没有同人正典文件。用 zworld fanfic init 创建同人书。`);
      }
