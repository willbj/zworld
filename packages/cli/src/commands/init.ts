...
export const initCommand = new Command("init")
  .description("Initialize an ZWorld project (current directory by default)")
  ...
  .action(async (name: string | undefined, opts: { lang?: string }) => {
    ...
    try {
      ...
      const exampleCreate = isEnglish
        ? "  zworld book create --title 'My Novel' --genre progression --platform royalroad --lang en"
        : "  zworld book create --title '我的小说' --genre xuanhuan --platform tomato";
      if (global) {
        ...
        log("  zworld config set-global --provider openai --base-url <your-api-url> --api-key <your-key> --model <your-model>");
        log("  # Option 2: Edit .env for this project only");
        log("");
        log(exampleCreate);
      }
      log("  zworld write next <book-id>");
    } catch (e) {
