...
import { listAvailableGenres, readGenreProfile, getBuiltinGenresDir } from "@zworld/core";
  ...
  .action(async (id: string) => {
    try {
      ...
      try {
        ...
        logError(`Built-in genre "${id}" not found. Use 'zworld genre list' to see available genres.`);
        process.exit(1);
        return;
      }
