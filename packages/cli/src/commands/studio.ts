...
export async function launchStudioWorkbench(root: string, port: string): Promise<void> {
  ...
  if (!launch) {
    logError(
      "ZWorld Studio not found. If you cloned the repo, run:\n" +
      "  cd packages/studio && pnpm install && pnpm build\n" +
      "Then run 'zworld studio' from the project root.",
    );
    process.exit(1);
  }

  log(`Starting ZWorld Studio on ${url}`);

  const child = spawn(launch.command, launch.args, {
    cwd: root,
    stdio: "inherit",
    env: { ...process.env, ZWORLD_STUDIO_PORT: port },
  });
...
export async function launchStudioEntry(
  ...
  if (prepared.initialized) {
    log(`No zworld.json found in ${root}. Initialized a minimal ZWorld project for Studio.`);
  }
...
export function createStudioCommand(hooks: StudioCommandHooks = {}): Command {
  ...
  .description("Start ZWorld Studio web workbench")
