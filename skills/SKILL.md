...
ZWorld is a story creation AI agent for long-form novels, short fiction, scripts, fan works, continuations, covers, and open-world / branching interactive play. Prefer the Studio Chat / action-surface workflow for natural-language requests: the model proposes or invokes typed actions, ZWorld executes them, and completion is derived from real tool results and files, not from prose claims.
...
Treat ZWorld as a confirmable action system, not a bag of prompt shortcuts.

- Natural-language requests should go through Studio Chat / TUI / `zworld interact` whenever possible.
...
v1.5 broadly improves instruction following, context management, weak-model formatting resilience, and error separation between ZWorld execution, provider calls, and image generation. Still surface unresolved audit issues plainly instead of claiming they were fixed.
...
zworld init my-writing-project
...
zworld config set-global --provider openai --base-url https://api.openai.com/v1 --api-key-env OPENAI_API_KEY --model gpt-4o
# For compatible/proxy endpoints, use --provider custom and point ONLY to trusted endpoints:
# zworld config set-global --provider custom --base-url https://your-trusted-proxy.com/v1 --api-key-env OPENAI_API_KEY --model gpt-4o
```

### Multi-Model Routing (Optional)
```bash
# Assign different models to different agents — balance quality and cost
zworld config set-model writer claude-sonnet-4-20250514 --provider anthropic --base-url https://api.anthropic.com --api-key-env ANTHROPIC_API_KEY
zworld config set-model auditor gpt-4o --provider openai
zworld config show-models
```
Agents without explicit overrides fall back to the global model.

### Provider setup tip

When the user needs an aggregator provider, point them to Studio's service settings. ZWorld includes kkaiapi and OpenRouter as optional aggregator choices. For kkaiapi, use:
...
1. **Initialize and create book**:
   ```bash
   zworld book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000
   # Or with a creative brief (your worldbuilding doc / ideas):
   zworld book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000 --brief my-ideas.md
   ```
...
2. **Generate initial chapters** (e.g., 5 chapters):
   ```bash
   zworld write next book-id --count 5 --words 3000 --context "young protagonist discovering powers"
   ```
...
3. **Review and approve chapters**:
   ```bash
   zworld review list book-id
   zworld review approve-all book-id
   ```

4. **Export the book** (supports txt, md, epub):
   ```bash
   zworld export book-id
   zworld export book-id --format epub
   ```
...
2. **Continue from last chapter**:
   ```bash
   zworld write next book-id --count 3 --words 2500 --context "protagonist faces critical choice"
   ```
   - ZWorld maintains structured runtime state, readable Markdown projections, control docs, and temporal memory for consistency
   - If only one book exists, omit `book-id` for auto-detection

3. **Review and approve**:
   ```bash
   zworld review approve-all
   ```

### Workflow 2.5: Shared Natural-Language Control (Recommended For OpenClaw)

When ZWorld is being driven by OpenClaw or another external agent, prefer the shared interaction executor instead of stitching together many ad-hoc CLI calls:

```bash
zworld interact --json --message "continue the current book, but keep the pacing tighter"
zworld interact --json --message "rewrite chapter 3"
zworld interact --json --message "start an open-world detective game about a locked archive room"
```
...
Use this when you have an existing novel (or partial novel) and want ZWorld to pick up where it left off.

1. **Import from a single text file** (auto-splits by chapter headings):
   ```bash
   zworld import chapters book-id --from novel.txt
   ```
...
2. **Import from a directory** of separate chapter files:
   ```bash
   zworld import chapters book-id --from ./chapters/
   ```
   - Reads `.md` and `.txt` files in sorted order

3. **Resume interrupted import**:
   ```bash
   zworld import chapters book-id --from novel.txt --resume-from 15
   ```

4. **Continue writing** from the imported chapters:
   ```bash
   zworld write next book-id --count 3
   ```
   - ZWorld rebuilds structured state, chapter summaries, hooks, character relationships, readable projections, and a style guide from the imported chapters
...
1. **Analyze reference text**:
   ```bash
   zworld style analyze reference_text.txt
   ```
   - Examines vocabulary, sentence structure, tone, pacing

2. **Import style to your book**:
   ```bash
   zworld style import reference_text.txt book-id --name "Author Name"
   ```
...
1. **Import parent canon**:
   ```bash
   zworld import canon spinoff-book-id --from parent-book-id
   ```
...
2. **Continue spinoff**:
   ```bash
   zworld write next spinoff-book-id --count 3 --context "alternate timeline after Chapter 20"
   ```
...
1. **Generate draft only**:
   ```bash
   zworld draft book-id --words 3000 --context "protagonist escapes" --json
   ```

2. **Audit the chapter** (33-dimension quality check):
   ```bash
   zworld audit book-id chapter-1 --json
   ```
...
3. **Revise with specific mode**:
   ```bash
   zworld revise book-id chapter-1 --mode polish --json
   ```
...
zworld detect book-id

# Deep scan all chapters
zworld detect book-id --all
```
...
```bash
zworld analytics book-id --json
# Shorthand alias
zworld stats book-id --json
```
...
zworld book create --title "The Last Delver" --genre litrpg --chapter-words 3000

# Or set language explicitly
zworld book create --title "My Novel" --genre other --lang en

# Set English as default for all projects
zworld config set-global --lang en
```
...
- Use `zworld genre list` to see all available genres

### Workflow 11: Fan Fiction

```bash
# Create a fanfic from source material
zworld fanfic init --title "My Fanfic" --from source-novel.txt --mode canon

# Modes: canon (faithful), au (alternate universe), ooc (out of character), cp (ship-focused)
zworld fanfic init --title "What If" --from source.txt --mode au --genre other
```
...
Use Studio Chat or `zworld interact` for small, explicit edits to generated text artifacts. Treat the chat layer like an external editor: it may edit persisted files, then the writing pipeline can continue from the updated state.

```bash
zworld interact --book my-book --json --message "把第 3 章里那句过长的解释删短一点，但不要改剧情事实"
zworld interact --json --message "把 covers/demo/cover-prompt.md 里的人物拉近一点，标题字更大"
```
...
When the user asks for a rename or targeted replacement, keep the request explicit and scoped. Prefer Studio Chat / `zworld interact` over hidden slash shortcuts; the agent should route the edit through the shared action surface and report which file changed.
...
- Shares the same interaction kernel as `zworld interact` and Studio
...
## ZWorld Studio (Web Workbench)

`zworld studio` launches a local web UI (default port 4567) that provides a visual interface for all ZWorld operations:
...
```bash
zworld studio              # Start on default port 4567
zworld studio -p 8080      # Start on custom port
```
...
```bash
zworld agent "写一部都市题材的小说，主角是一个年轻律师，第一章三千字"
```
...
These are the preferred tools when ZWorld is driven by OpenClaw, Studio chat, or `zworld agent`:
...
```bash
# Explicit
zworld write next book-123 --count 1

# Auto-detected (if only one book exists)
zworld write next --count 1
```

### --json Flag
All content-generating commands support `--json` for structured output. Essential for programmatic use:
```bash
zworld draft book-id --words 3000 --context "guidance" --json
```

### Long-Term Memory
ZWorld maintains each book's memory in three layers:
...
Agents use the governed composer to select the relevant parts for the current task instead of blindly injecting every file. During `import chapters`, ZWorld rebuilds structured state and projections from existing content via the ChapterAnalyzerAgent.
...
```bash
zworld write next book-id --count 2 --context "protagonist discovers betrayal, must decide whether to trust mentor"
```
...
```bash
zworld genre list
zworld genre show xuanhuan
```

### Create Custom Genre
```bash
zworld genre create my-genre --name "My Genre"
# Options: --numerical, --power, --era
zworld genre create dark-xuanhuan --name "Dark Xuanhuan" --numerical --power
```

### Copy Built-in Genre for Customization
```bash
zworld genre copy xuanhuan
...
|---------|---------|-------|
| `zworld init [name]` | Initialize project | One-time setup |
| `zworld book create` | Create new book | Returns book-id. `--brief <file>`, `--lang en/zh`, `--genre litrpg/progression/...` |
| `zworld book list` | List all books | Shows IDs, statuses |
| `zworld write next` | Full pipeline (draft→audit→revise) | Primary workflow command |
| `zworld draft` | Generate draft only | No auditing/revision |
| `zworld audit` | 33-dimension quality check | Standalone evaluation |
| `zworld revise` | Revise chapter | Modes: polish/spot-fix/rewrite/rework/anti-detect |
| `zworld agent` | Natural language interface | Flexible requests |
| `zworld style analyze` | Analyze reference text | Extracts style profile |
| `zworld style import` | Apply style to book | Makes style permanent |
| `zworld import canon` | Link spinoff to parent | For prequels/sequels |
| `zworld import chapters` | Import existing chapters | Reverse-engineers truth files for continuation |
| `zworld detect` | AIGC detection | Flags AI-generated passages |
| `zworld export` | Export finished book | Formats: txt, md, epub |
| `zworld analytics` / `zworld stats` | View book statistics | Word count, audit rates, token usage |
| `zworld radar scan` | Platform trend analysis | Informs new book ideas |
| `zworld short run` | Generate standalone short fiction | Outputs manuscript, sales package, cover prompt, optional cover |
| `zworld config set-global` | Configure LLM provider | OpenAI/Anthropic/custom (any OpenAI-compatible) |
| `zworld config set-model <agent> <model>` | Set model override for a specific agent | `--provider`, `--base-url`, `--api-key-env` for multi-provider routing |
| `zworld config show-models` | Show current model routing | View per-agent model assignments |
| `zworld doctor` | Diagnose issues | Check installation |
| `zworld update` | Update to latest version | Self-update |
| `zworld up/down` | Daemon mode | Background processing. Logs to `zworld.log` (JSON Lines). `-q` for quiet mode |
| `zworld review list/approve-all` | Manage chapter approvals | Quality gate |
| `zworld fanfic init` | Create fanfic from source material | `--from <file>`, `--mode canon/au/ooc/cp` |
| `zworld genre list` | List all available genres | Shows English and Chinese genres with default language |
| `zworld genre create <id>` | Create custom genre profile | `--name`, `--numerical`, `--power`, `--era` |
| `zworld genre copy <id>` | Copy built-in genre to project | For customization |
| `zworld write rewrite <book> <ch>` | Rewrite a specific chapter | Deletes chapter and later, rewrites from that point |
| `zworld book update [book-id]` | Update book settings | `--chapter-words`, `--target-chapters`, `--status`, `--lang` |
| `zworld book delete <book-id>` | Delete book and all chapters | `--force` to skip confirmation |
| `zworld plan chapter [book-id]` | Generate chapter intent | Preview what next chapter will do before writing |
| `zworld compose chapter [book-id]` | Generate runtime artifacts | Context, rule-stack, trace for next chapter |
| `zworld consolidate [book-id]` | Consolidate chapter summaries | Reduces context for long books (volume-level summaries) |
| `zworld eval [book-id]` | Quality evaluation report | `--json`, `--chapters <range>`. Composite quality score |
| `zworld` / `zworld studio` | Start web workbench | `-p` for port. Local web UI for book management |
| `zworld fanfic show [book-id]` | Display parsed fanfic canon | Shows imported source material analysis |
| `zworld fanfic refresh [book-id]` | Re-import and regenerate fanfic canon | `--from <file>` for updated source material |
| `zworld interact` | Shared interaction endpoint | `--json`, `--message`, `--book`. Primary entry for OpenClaw |
| `zworld tui` | Launch TUI dashboard | Full-screen Ink + React interactive dashboard |
...
**"book-id not found"**
- Verify the ID with `zworld book list`
- Ensure you're in the correct project directory

**"Provider not configured"**
- Run `zworld config set-global` with valid credentials
- Check API key and base URL with `zworld doctor`
...
- Try `zworld revise` with `--mode rewrite`
...
2. **Start with style**: If imitating an author, run `zworld style import` before generation
3. **Import first**: For existing novels, use `zworld import chapters` to bootstrap truth files before continuing
4. **Review regularly**: Use `zworld review` to catch issues early
5. **Monitor audits**: Check `zworld audit` metrics to understand quality bottlenecks
...
8. **Check analytics**: Use `zworld analytics` to track quality trends over time
9. **Export frequently**: Keep backups with `zworld export`
...
- **Credential handling**: prefer Studio service settings or `--api-key-env <VAR_NAME>` over literal keys. Studio stores service secrets in project-local `.zworld/secrets.json`; CLI env settings live in `~/.zworld/.env` or project `.env`. Treat all of these as secrets and keep them out of commits.
...
- **No elevated privileges**: ZWorld requires no sudo, no global state mutation, no network listening port (Studio binds `localhost:4567` only).

## Support & Resources

- **Homepage**: https://github.com/Narcooo/zworld
- **Configuration**: Stored in project root after `zworld init`
- **Truth files**: Located in `books/<id>/story/` per book, with structured JSON in `story/state/`
- **Logs**: Check output of `zworld doctor` for troubleshooting
